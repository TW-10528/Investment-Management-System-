...
export default function Dashboard() {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      {/* ── Header (Static) ── */}
      <div className="flex-shrink-0 bg-white border-b" style={{ borderColor: 'var(--color-card-border)' }}>
      ...
      {/* ── Scrollable Content ── */}
      <div className="flex-1 overflow-y-auto overflow-x-hidden p-5 space-y-3 animate-fade-in">
      {/* ── Overdue alert ── */}
      ...
      <div>
        ...
        <div className="grid grid-cols-5 gap-4">
          {(() => {
            ...
            const cards = [
              { emoji: '📋', label: t('metrics.commitment'), value: fmt.usdFull(regularCommit), currency: 'USD' },
              { emoji: '📈', label: t('metrics.distributions'), value: fmt.usdFull(regularDist), currency: 'USD' },
            ];

            if (totals.regularReturnOfCapital !== 0) {
              cards.push({ emoji: '💸', label: t('metrics.returnOfCapital'), value: fmt.usdFull(totals.regularReturnOfCapital), currency: 'USD' });
            }
            if (totals.regularGain !== 0) {
              cards.push({ emoji: '📊', label: t('metrics.gain'), value: fmt.usdFull(totals.regularGain), currency: 'USD' });
            }
            if (totals.regularInterest !== 0) {
              cards.push({ emoji: '📌', label: t('metrics.interest'), value: fmt.usdFull(totals.regularInterest), currency: 'USD' });
            }
      ...
      {(() => {
        ...
        const cards = [
          { emoji: '📋', label: t('metrics.commitment'), value: fmt.jpy(sdgCommit), currency: 'JPY' },
          { emoji: '📈', label: t('metrics.distributions'), value: latestSaved ? fmt.jpy(sdgDist * latestSaved) : fmt.jpy(sdgDist), currency: 'JPY' },
        ];

        if (totals.sdgInterest !== 0) {
          cards.push({ emoji: '📌', label: t('metrics.interest'), value: fmt.jpy(totals.sdgInterest), currency: 'JPY' });
        }
      ...
      <div className="border-t border-gray-200 pt-4 mt-2">
        ...
        {(() => {
          ...
          const kpis = [
            { icon: '🎯', label: t('metrics.moic'), value: moic.toFixed(2) + '×', subtitle: `DPI ${dpi.toFixed(2)}× · TVPI ${tvpi.toFixed(2)}×` },
            { icon: '📊', label: t('metrics.netIRR'), value: data.irr != null ? `${data.irr.toFixed(1)}%` : '—', subtitle: t('dashboard.sinceInception') },
            { icon: '💰', label: t('metrics.dryPowder'), value: fmt.usdFull(dryPowder), subtitle: `${((dryPowder/regularCommit)*100).toFixed(2)}% ${t('dashboard.available')}` },
            { icon: '🏦', label: t('metrics.totalNavUnreal'), value: fmt.usdFull(regularNav), subtitle: t('manageFunds.latestReported') },
            { icon: '🎯', label: t('metrics.moic') + ' (JPY)', value: sdgMoic.toFixed(2) + '×', subtitle: `DPI ${sdgDpi.toFixed(2)}× · TVPI ${sdgTvpi.toFixed(2)}×` },
            { icon: '📊', label: t('metrics.netIRR') + ' (JPY)', value: '—', subtitle: t('dashboard.sinceInception') },
            { icon: '💰', label: t('metrics.dryPowder'), value: latestSaved ? fmt.jpy(sdgDryPowder * latestSaved) : fmt.jpy(sdgDryPowder), subtitle: `${((sdgDryPowder/sdgCommit)*100).toFixed(2)}% ${t('dashboard.available')}` },
            { icon: '🏦', label: t('metrics.totalNavUnreal'), value: latestSaved ? fmt.jpy(sdgNav * latestSaved) : fmt.jpy(sdgNav), subtitle: t('manageFunds.latestReported') },
          ];
