...
import { exportFundsToExcel } from '../lib/export';
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end">
          {section === 'manage' && !selectedFund && (
            <>
              <button onClick={() => {
                const transactions = funds.map(f => ({
                  'Fund Name': f.fund_name,
                  'Manager': f.manager || '—',
                  'Commitment (USD)': f.commitment_usd || 0,
                  'Distribution (USD)': f.total_received_usd || 0,
                  'Currency': f.currency || 'USD',
                }));
                exportFundsToExcel(funds, transactions, `funds-${new Date().toISOString().split('T')[0]}.xlsx`);
                toast.success('Excel file downloaded');
              }}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border theme-divider theme-text-muted hover:theme-text transition-colors">
                ⬇️ Export
              </button>
              {canEdit && (
                <button onClick={() => setShowWizard(true)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-indigo-600 hover:bg-indigo-700 text-white transition-colors whitespace-nowrap">
                  + {t('funds.addFund')}
                </button>
              )}
            </>
