...
import { fundsAPI, fxRatesAPI, fundReportsAPI, dashboardAPI } from '../services/api';
...
import { exportFundsToExcel } from '../utils/exportToExcel';
...
function DetailsTab({ detail, canEdit, fundId, onSaved }: { detail: FundDetail; canEdit:boolean; fundId:string; onSaved:()=>void }) {
  ...
  function startEdit() {
    setForm({
      ...
      vintage_year:            String(detail.vintage_year??''),
      currency:                detail.currency??'USD',
      commitment_usd:          isSdg ? undefined : String(detail.commitment_usd??''),
      commitment_jpy:          isSdg ? String(detail.commitment_jpy??'') : undefined,
      contract_commitment_jpy: isSdg ? String(detail.contract_commitment_jpy??'') : undefined,
      entry_fx_rate:           String(detail.entry_fx_rate??''),
      ...
      fund_term_years:         String(detail.fund_term_years??''),
      management_fee_pct:      String(detail.management_fee_pct??''),
      carry_pct:               String(detail.carry_pct??''),
      hurdle_rate_pct:         String(detail.hurdle_rate_pct??''),
      notes:                   detail.notes??'',
    });
    setEditing(true);
  }
...
export default function FundManagement() {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      console.log('✓ Dashboard summary fetched:', summary.fund_summaries.length, 'funds');

      console.log('2️⃣ Fetching FX rates...');
      const fxRes = await fxRatesAPI.latest();
      const rate = fxRes.data?.usd_jpy ?? 0;
      console.log('✓ FX rate fetched:', rate);

      console.log('3️⃣ Fetching ledger data for', funds.length, 'funds...');
      ...
      await Promise.all(funds.map(async f => {
        if (f.is_active !== false) {
          try {
            ...
            ledgers[f.fund_id] = ledgerRes.data?.rows ?? [];
            console.log(`  ✓ Ledger for ${f.fund_name}:`, ledgers[f.fund_id].length, 'rows');
          } catch (e) {
            console.warn(`  ⚠ Failed to fetch ledger for ${f.fund_name}:`, e);
          }
        }
      }));

      console.log('4️⃣ Calling exportFundsToExcel...');
      // Call export function
      await exportFundsToExcel({
        summary,
        funds,
        details,
        ledgers,
      ...
      toast.success('Fund data exported successfully');
    } catch (e: any) {
      console.error('❌ Export error:', e);
      toast.error(e?.message ?? 'Failed to export fund data');
    } finally {
      setExporting(false);
    }
  }, [funds, details]);
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
          ...
          {section === 'manage' && !selectedFund && (
            <>
              <button onClick={handleExport} disabled={exporting}
                className="px-4 py-2 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 text-sm font-semibold rounded-lg transition-colors flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {/* Scrollable Content */}
      <div className="flex-1 overflow-y-auto overflow-x-hidden p-6 space-y-6 animate-fade-in">
      {/* View-only banner */}
      {!canEdit && (
