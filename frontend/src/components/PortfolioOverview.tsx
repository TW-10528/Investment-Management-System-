...
import { Bar, BarChart, PieChart, Pie, Cell, XAxis, YAxis, Tooltip, Legend, CartesianGrid, ResponsiveContainer } from 'recharts';
...
export default function PortfolioOverview({ onSelectFund }: { onSelectFund?: (id: string) => void }) {
  ...
  return (
    <div className="space-y-6">
      {/* ── Header with Toggle ── */}
      <div className="flex items-center justify-between px-5 py-4 bg-white rounded-lg border" style={{ borderColor: 'rgba(71,85,105,0.16)' }}>
        <h1 className="text-xl font-bold" style={{ color: '#1e40af' }}>{t('manageFunds.portfolioSummary')}</h1>
        <button
          onClick={() => setSidebarOpen(!sidebarOpen)}
          className="lg:hidden flex items-center justify-center w-9 h-9 rounded-lg transition-all duration-200 hover:scale-105 flex-shrink-0"
          style={{
            background: 'white',
      ...
      <div>
        <h2 className="text-sm font-bold mb-3" style={{ color: '#1e40af' }}>{t('manageFunds.portfolioSummary')}</h2>
        <div className="space-y-4">
          {/* 7 Regular Funds in USD */}
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: '#475569' }}>7 Funds (USD)</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              {(() => {
                ...
                const tileData = [
                  { emoji: '📋', label: t('manageFunds.totalCommitmentUsd'), value: regularCommit, change: '+3.25%' },
                  { emoji: '📈', label: t('manageFunds.totalContributionUsd'), value: regularDist, change: '+3.25%' },
                  { emoji: '💸', label: t('manageFunds.returnOfCapitalUsd'), value: regularRoc, change: '+1.18%' },
                  { emoji: '📊', label: t('metrics.gain'), value: regularGain, change: '+2.08%' },
                  { emoji: '📌', label: t('manageFunds.interestUsd'), value: regularInterest, change: '+4.32%' },
                ];

                return tileData.map((tile, idx) => (
                  <div key={idx} className="border rounded-lg p-3" style={{ minHeight: '110px', background: 'white', borderColor: 'rgba(71,85,105,0.16)' }}>
                    <div className="flex items-start gap-2 mb-2">
                      <span className="text-lg">{tile.emoji}</span>
                      <p className="text-[8px] font-bold uppercase tracking-widest" style={{ color: '#475569' }}>{tile.label}</p>
                    </div>
                    <p className="text-base font-bold tabular-nums mb-2" style={{ color: '#1e40af' }}>{fmt.usdFull(tile.value)}</p>
                    <div className="flex items-center justify-between">
                      <p className="text-[9px]" style={{ color: '#475569' }}>USD</p>
                      <p className="text-[9px] font-semibold" style={{ color: '#10b981' }}>{tile.change}</p>
                    </div>
                    <p className="text-[8px] mt-0.5" style={{ color: '#475569' }}>vs Last Month</p>
                  </div>
                ));
              })()}
            </div>
          ...
          {/* SDG Fund in JPY */}
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: '#475569' }}>{t('manageFunds.sdgFundJpy')}</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              {(() => {
                ...
                const tileData = [
                  { emoji: '📋', label: t('manageFunds.sdgCommitmentJpy'), value: sdgCommit, change: '+1.66%' },
                  { emoji: '📈', label: t('manageFunds.sdgContributionJpy'), value: sdgDist, change: '+1.66%' },
                  { emoji: '📌', label: t('manageFunds.sdgInterestJpy'), value: sdgInterest, change: '+2.91%' },
                ];
                ...
                return tileData.map((tile, idx) => (
                  <div key={idx} className="border rounded-lg p-3" style={{ minHeight: '110px', background: 'white', borderColor: 'rgba(71,85,105,0.16)' }}>
                    ...
                    <p className="text-base font-bold tabular-nums mb-2" style={{ color: '#1e40af' }}>{fmt.jpy(tile.value)}</p>
                    <div className="flex items-center justify-between">
                      <p className="text-[9px]" style={{ color: '#475569' }}>JPY</p>
                      <p className="text-[9px] font-semibold" style={{ color: '#10b981' }}>{tile.change}</p>
                    </div>
                    <p className="text-[8px] mt-0.5" style={{ color: '#475569' }}>vs Last Month</p>
                  </div>
      ...
      {activeFunds.length > 0 && (() => {
        ...
        return (
          <div>
            <h2 className="text-sm font-bold mb-3" style={{ color: '#1e40af' }}>{t('manageFunds.fundCalculations')}</h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {/* ROW 1: Bar USD (LEFT) + Pie+Table USD (RIGHT) */}
              {/* Bar — 7 Funds commitment / contribution / distribution (USD) */}
              <div className="border rounded-2xl p-4" style={{ background: 'white', borderColor: 'rgba(71,85,105,0.16)' }}>
                <p className="text-sm font-bold mb-1" style={{ color: '#1e40af' }}>{t('manageFunds.commitmentContributionDistribution')}</p>
                <div style={{ width: '100%', height: 350 }}>
                  <ResponsiveContainer>
                    <BarChart data={barDataUsd} margin={{ top: 40, right: 8, bottom: 60, left: 4 }}>
                      ...
                      <Bar dataKey="Commitment" fill="#2a78d6" radius={[3,3,0,0]} label={{ dataKey: 'Commitment', formatter: (v: any) => fmt.usdAbbr(v ?? 0), position: 'top', fontSize: 9, fill: '#2a78d6' }} />
                      <Bar dataKey="Contribution" fill="#008300" radius={[3,3,0,0]} label={{ dataKey: 'Contribution', formatter: (v: any) => fmt.usdAbbr(v ?? 0), position: 'top', fontSize: 9, fill: '#008300' }} />
                      <Bar dataKey="Distribution" fill="#eda100" radius={[3,3,0,0]} label={{ dataKey: 'Distribution', formatter: (v: any) => fmt.usdAbbr(v ?? 0), position: 'top', fontSize: 9, fill: '#eda100' }} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="border rounded-2xl p-4" style={{ background: 'white', borderColor: 'rgba(71,85,105,0.16)' }}>
                <p className="text-sm font-bold mb-3" style={{ color: '#1e40af' }}>{t('manageFunds.commitmentAllocationUsd')}</p>
              ...
              {barDataJpy.length > 0 && (
                <div className="theme-card border theme-border rounded-2xl p-4">
                  <p className="text-sm font-bold theme-text mb-1">{t('manageFunds.sdgFundActivityJpy')}</p>
                  <div style={{ width: '100%', height: 350 }}>
                    <ResponsiveContainer>
                      <BarChart data={barDataJpy} margin={{ top: 40, right: 8, bottom: 60, left: 4 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--color-card-border)" vertical={false} />
                        <XAxis dataKey="name" tick={{ fontSize: 10, fill: 'var(--color-text-muted)' }} interval={0} angle={-20} textAnchor="end" height={80} />
                        ...
                        <Legend wrapperStyle={{ fontSize: 11 }} />
                        <Bar dataKey="Commitment" fill="#2a78d6" radius={[3,3,0,0]} label={{ dataKey: 'Commitment', formatter: (v: any) => fmt.jpy(v ?? 0), position: 'top', fontSize: 10, fill: '#2a78d6' }} />
                        <Bar dataKey="Contribution" fill="#008300" radius={[3,3,0,0]} label={{ dataKey: 'Contribution', formatter: (v: any) => fmt.jpy(v ?? 0), position: 'top', fontSize: 10, fill: '#008300' }} />
                        <Bar dataKey="Distribution" fill="#eda100" radius={[3,3,0,0]} label={{ dataKey: 'Distribution', formatter: (v: any) => fmt.jpy(v ?? 0), position: 'top', fontSize: 10, fill: '#eda100' }} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              {pieDataJpy.length > 0 && sdgFund && (
                <div className="theme-card border theme-border rounded-2xl p-4">
                  <p className="text-sm font-bold theme-text mb-3">{t('manageFunds.sdgCommitmentAllocationJpy')}</p>
