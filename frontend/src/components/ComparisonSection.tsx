  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold theme-text">{t('manageFunds.fundSeriesComparison')}</h1>
          {selectedSeries && (
            <p className="text-sm theme-text-muted mt-1">{t('manageFunds.comparePerformance')} <span className="font-semibold theme-text">{selectedSeries}</span> series</p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-4 justify-between">
        <div className="flex items-center gap-3 flex-wrap">
          <label className="text-sm font-semibold theme-text min-w-fit">{t('manageFunds.series')}:</label>
          <select
          ...
          >
            <option value="">{t('manageFunds.selectSeries')}</option>
            {seriesGroups.map(g => (
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm theme-text-muted">{t('manageFunds.asOf')} {formatDate(asOfDate)}</span>
      ...
      {selectedSeries.length === 0 ? (
        <div className="theme-card border theme-border rounded-2xl py-24 text-center">
          <p className="text-5xl mb-4 opacity-20">📊</p>
          <p className="text-base font-medium theme-text">{t('manageFunds.selectASeries')}</p>
          <p className="text-sm theme-text-muted mt-1">{t('manageFunds.chooseFundSeries')}</p>
        </div>
        ...
        <div className="theme-card border theme-border rounded-2xl py-24 text-center">
          <p className="text-5xl mb-4 opacity-20">📊</p>
          <p className="text-base font-medium theme-text">{t('manageFunds.loadingComparison')}</p>
          <p className="text-sm theme-text-muted mt-1">{t('manageFunds.fetchingFundData')} {selectedSeries.join(', ')}</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 pt-20">
          <div className="theme-card border theme-border rounded-2xl shadow-2xl w-full max-w-md max-h-[70vh] flex flex-col overflow-hidden animate-fade-in">
            ...
            <div className="px-6 py-4 border-b theme-border flex items-center justify-between">
              <h2 className="text-lg font-bold theme-text">{t('manageFunds.addSeriesToComparison')}</h2>
              <button
...
function SeriesComparison({
  ...
  const kpiCards = useMemo(() => [
    { label: t('dashboard.totalValue'), value: fmt.usdFull(totals.totalAum), count: selectedFunds.length, icon: ICONS.aum },
    { label: t('manageFunds.totalCommitmentComparison'), value: fmt.usdFull(totals.totalCommitment), count: selectedFunds.length, icon: ICONS.commitment },
    { label: t('manageFunds.totalDistributionComparison'), value: fmt.usdFull(totals.totalDistribution), count: selectedFunds.length, icon: ICONS.distribution },
    { label: t('metrics.returnOfCapital') + ' (USD)', value: fmt.usdFull(totals.totalRoc), count: selectedFunds.length, icon: ICONS.roc },
    { label: t('metrics.gain') + ' (USD)', value: fmt.usdFull(totals.totalGain), count: selectedFunds.length, icon: ICONS.gain },
    { label: t('metrics.interest') + ' (USD)', value: fmt.usdFull(totals.totalInterest), count: selectedFunds.length, icon: ICONS.interest },
  ], [t, totals.totalAum, totals.totalCommitment, totals.totalDistribution, totals.totalRoc, totals.totalGain, totals.totalInterest, selectedFunds.length]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Metrics Table */}
      <div>
        <h3 className="text-sm font-bold theme-text mb-3">{t('manageFunds.keyMetricsComparison')}</h3>
          ...
          <table className="w-full text-xs">
            <thead style={{ background: 'var(--color-header-bg)' }}>
              <tr className="border-b theme-border text-[10px] uppercase tracking-wide theme-text-muted">
                {[
                  { key: 'fundName', label: t('manageFunds.fundName') },
                  { key: 'vintageYear', label: t('manageFunds.vintageYear') },
                  { key: 'currency', label: t('manageFunds.currency') },
                  { key: 'aumUsd', label: t('manageFunds.aumUsd') },
                  { key: 'commitmentUsd', label: t('manageFunds.commitmentUsd') },
                  { key: 'distributedUsd', label: t('manageFunds.distributedUsd') },
                  { key: 'returnOfCapitalUsd', label: t('manageFunds.returnOfCapitalUsd') },
                  { key: 'gainUsd', label: t('manageFunds.gainUsd') },
                  { key: 'interestUsd', label: t('manageFunds.interestUsd') },
                  { key: 'irr', label: t('manageFunds.irr') },
                  { key: 'moic', label: t('manageFunds.moic') },
                  { key: 'dpi', label: t('manageFunds.dpi') },
                  { key: 'tvpi', label: t('manageFunds.tvpi') },
                ].map(h => (
                  <th key={h.key} className={`px-3 py-2 font-semibold whitespace-nowrap ${h.key === 'fundName' ? 'text-left' : 'text-right'}`}>{h.label}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y theme-border">
              ...
              <tr style={{ background: 'rgba(99,102,241,0.05)' }}>
                <td className="px-3 py-2 text-sm font-bold theme-text">{t('manageFunds.totalAverage')}</td>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="theme-card border theme-border rounded-xl p-4">
          <p className="text-sm font-bold theme-text mb-4">{t('manageFunds.aumComparison')}</p>
        ...
        <div className="theme-card border theme-border rounded-xl p-4">
          <p className="text-sm font-bold theme-text mb-4">{t('manageFunds.performanceComparison')}</p>
          <div style={{ width: '100%', height: 300 }}>
            <ResponsiveContainer>
        ...
        <div className="theme-card border theme-border rounded-xl p-4">
          <p className="text-sm font-bold theme-text mb-4">{t('manageFunds.returnsMultiplesComparison')}</p>
          <div style={{ width: '100%', height: 300 }}>
            <ResponsiveContainer>
        ...
        <div className="theme-card border theme-border rounded-xl p-4">
          <p className="text-sm font-bold theme-text mb-4">{t('manageFunds.cumulativeCashFlow')}</p>
          <div style={{ width: '100%', height: 300 }}>
            <ResponsiveContainer>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div>
          <h3 className="text-sm font-bold theme-text mb-3">{t('manageFunds.quickStats')}</h3>
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: t('manageFunds.averageIrr'), value: '12.45%', color: C.emerald },
              { label: t('manageFunds.averageMoic'), value: '1.32×', color: C.indigo },
              { label: t('manageFunds.averageDpi'), value: '0.58×', color: C.indigo },
              { label: t('manageFunds.averageTvpi'), value: '0.99×', color: C.indigo },
            ].map(m => (
              <div key={m.label} className="theme-card border theme-border rounded-xl p-3">
        ...
        <div>
          <h3 className="text-sm font-bold theme-text mb-3">{t('manageFunds.topPerformer')}</h3>
          <div className="theme-card border theme-border rounded-xl p-4 space-y-3">
            {[
              { label: t('manageFunds.highestIrr'), value: 'Hamilton Lane Secondary Fund VII (0.80k)' },
              { label: t('manageFunds.highestMoic'), value: 'Hamilton Lane Secondary Fund VI-B (1.38x)' },
              { label: t('manageFunds.highestDpi'), value: 'Hamilton Lane Secondary Fund VI-B (0.58x)' },
              { label: t('manageFunds.highestTvpi'), value: 'Hamilton Lane Secondary Fund VI-B (1.02x)' },
            ].map(m => (
              <div key={m.label}>
                <p className="text-[9px] font-bold uppercase tracking-widest theme-text-muted mb-1">{m.label}</p>
