...
export async function validateExtractedNotice(
  ...
  const grossAmounts    = pastCalls.map((c: any) => parseFloat(c.grossCallUsd.toString()))
  const mgmtFees        = pastCalls.map((c: any) => parseFloat((c.managementFeeUsd ?? 0).toString()))
  const callPcts        = pastCalls.map((c: any) => parseFloat((c.callPct ?? 0).toString()))
  const lastCall        = pastCalls[pastCalls.length - 1] ?? null
  const lastCallNumber  = lastCall?.callNumber ?? 0
  const cumulativePct   = callPcts.reduce((s: number, p: number) => s + p, 0)
  ...
  if (noticeType === 'distribution') {
    ...
    const totalCalled = pastCalls.reduce((s: number, c: any) => s + parseFloat(c.grossCallUsd.toString()), 0)
    const totalDisted  = pastDists.reduce((s: number, d: any) => s + parseFloat(d.amountUsd.toString()), 0)
