...
function parseOcrStdout(stdout: string): string {
 ...
 */
export async function paddleOcrImageBuffers(buffers: Buffer[], lang: PaddleOcrLang = 'japan'): Promise<string> {
  if (!buffers.length) return ''

  if (!fs.existsSync(VENV_PYTHON)) {
    console.error('[paddleOcr] venv not found at', VENV_PYTHON,
      '— run: python3 -m venv paddle-venv && paddle-venv/bin/pip install paddlepaddle paddleocr')
    return ''
  }
  ...
  const tmpFiles = buffers.map((buf, i) => {
    if (!buf || buf.length === 0) return null   // skip zero-byte pages
    const f = path.join(os.tmpdir(), `${prefix}-p${i}.png`)
  ...
  try {
    console.log(`[paddleOcr] calling Python script: ${VENV_PYTHON} ${SCRIPT_PATH} with ${tmpFiles.length} file(s)`)
    const { stdout, stderr } = await execFileAsync(
      VENV_PYTHON,
      [SCRIPT_PATH, ...tmpFiles, `--lang=${lang}`],
      { timeout: TIMEOUT_MS, maxBuffer: 32 * 1024 * 1024 },
    )
    console.log(`[paddleOcr] subprocess returned: ${stdout?.length ?? 0} bytes stdout, ${stderr?.length ?? 0} bytes stderr`)
    if (stderr) console.warn(`[paddleOcr] stderr: ${stderr.slice(0, 200)}`)
    return parseOcrStdout(stdout)
  } catch (err: any) {
    const detail = (err as any).stdout?.trim() || ''
    const stderr = (err as any).stderr?.trim() || ''
    console.error('[paddleOcr] subprocess failed:', err?.message ?? err,
      `| code: ${err?.code ?? 'unknown'}`,
      detail ? `| python stdout: ${detail.slice(-300)}` : '',
 ...
 */
export async function paddleOcrImage(buffer: Buffer, lang: PaddleOcrLang = 'japan'): Promise<string> {
