...
import { extractGoldmanVintageXReport } from './extractor'
import type { GoldmanPreviousState } from './types'
import type { ParsedFundNotice } from '../types'

export { extractGoldmanVintageXReport }
export type { GoldmanReport, GoldmanPreviousState } from './types'

export function parseGoldmanSachs(
  rawText: string,
  previousState: GoldmanPreviousState | null = null,
): ParsedFundNotice {
  const report = extractGoldmanVintageXReport(rawText, '', previousState)
  ...
  const investmentTargets: { projectName: string }[] = []
  const seen = new Set<string>()
  for (const m of rawText.matchAll(/Project\s+([A-Z][a-zA-Z]+)/g)) {
    const name = `Project ${m[1]}`
    if (!seen.has(name)) { seen.add(name); investmentTargets.push({ projectName: name }) }
  }
  ...
  if (investmentTargets.length)  score++
