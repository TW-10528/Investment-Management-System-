...
export function parseNbRealEstate(rawText: string, previousState: NbPreviousState | null = null): ParsedFundNotice {
  const report = extractNbRealestateReport(rawText, '', previousState)
  ...
  let score = 0
  if (a.payment_date)                 score += 2
  if (commitmentUsd > 0)              score += 2
  if (grossCallUsd > 0)               score += 2
  if (distributionUsd > 0)            score++
