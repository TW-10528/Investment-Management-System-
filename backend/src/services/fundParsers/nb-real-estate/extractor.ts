...
import type {
  NbAllFields, NbBreakdown, NbBreakdownItem, NbCalculatedFields,
  NbCalculationResult, NbExcelFields, NbPreviousState, NbRealEstateReport,
  NbValidation,
} from './types'
...
function findCompanyName(text: string): string | null {
  ...
  if (byOrder) return byOrder[1].trim().split(/\s+/).join(' ')
  return null
}
...
function extractAllFields(text: string): NbAllFields {
  ...
  let additionalPaymentDueToSubsequentClosing: number | null = null
  const addlMatch = text.match(new RegExp(
    'Additional\\s+Payment\\s*\\(Pursuant\\s+to\\s+Section\\s+6\\.7\\(c\\)\\s+of\\s+the\\s+Limited\\s+Partnership\\s+Agreement\\)\\*{0,5}\\s*' + AMOUNT,
    'i',
  ))
  if (addlMatch) additionalPaymentDueToSubsequentClosing = cleanAmount(addlMatch[1], false)
  ...
  if (managementFeeAmount != null) netManagementFee = managementFeeAmount - (managementFeeRebate || 0)

  let currentGrossCapitalContribution: number | null = 0
  if (lpShareCapitalContribution != null) currentGrossCapitalContribution += lpShareCapitalContribution
  currentGrossCapitalContribution += netManagementFee || 0
  if (currentGrossCapitalContribution === 0) currentGrossCapitalContribution = null
...
function buildBreakdown(a: NbAllFields): NbBreakdown {
  const capital_call_breakdown: NbBreakdownItem[] = []
  const distribution_breakdown: NbBreakdownItem[] = []
...
function calculateCurrentTransactionCashFlow(capitalContributionAmount: number, distributionAmountReceived: number): number {
  return round2(-(capitalContributionAmount || 0) + (distributionAmountReceived || 0))
}

function mapToExcelFields(a: NbAllFields, breakdown: NbBreakdown): NbExcelFields {
  ...
  if (a.management_fee_rebate) remarksParts.push('Management fee rebate deducted from capital contribution amount.')
  if (a.additional_payment_received) remarksParts.push('Additional payment received included in distribution amount received, but not reinvestable amount.')
  if (taxExpense) remarksParts.push('Tax expense extracted separately and not used in cash flow.')
...
function calculateExcelFields(
  extracted: NbExcelFields,
  a: NbAllFields,
  previousState: NbPreviousState | null = null,
): NbCalculationResult {
  ...
  const calculatedFields: NbCalculatedFields = {
...
function buildValidation(
  excelFields: NbExcelFields,
  a: NbAllFields,
  breakdown: NbBreakdown,
  calculationResult: NbCalculationResult,
): NbValidation {
  const requiredExcelFields: (keyof NbExcelFields)[] = [
  ...
  const capitalCallBreakdownTotal = round2(
    breakdown.capital_call_breakdown.reduce((s, i) => s + (i.amount ?? 0), 0),
  )
...
export function extractNbRealestateReport(
  rawText: string,
  fileName = '',
  previousState: NbPreviousState | null = null,
): NbRealEstateReport {
  ...
  const finalExcelFields: NbExcelFields = { ...excelFields }
