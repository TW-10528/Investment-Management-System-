...
class FundFamilyService {
  ...
  async getAllFamiliesWithMembers() {
    ...
    existingFunds.forEach((fund: any) => {
    ...
    const allFamilies = families.map((f: any) => ({
      ...f,
      funds: [...(familiesByName[f.familyName]?.funds || []), ...f.funds],
    }));

    // Add existing families that don't have new funds
    Object.values(familiesByName).forEach((existingFamily: any) => {
      if (!allFamilies.find((f: any) => f.familyName === existingFamily.familyName)) {
