...
export interface DistributionBreakdown {
  ...
  totalUsd?:         number
  sources?:          Array<{ name: string; capitalReturn: number; gain: number; total: number }>
}

export interface ExtractedNotice {
  noticeType:      'capital_call' | 'distribution' | 'financial_statement'
  isCombined?:     boolean   // true for Capital Call + Deemed Distribution in one notice
  confidence:      number
  confidenceGrade: 'high' | 'medium' | 'low'
  fundName?:       string
  lpId?:           string    // LP investor ID
  amounts:         number[]
  dates:           string[]
  keywords:        string[]
  ...
  grossCallUsd?:       number
  netCallUsd?:         number
  deemedDistUsd?:      number   // distribution offset in a combined notice
  reinvestableUsd?:    number
  managementFeeUsd?:   number
  expenseUsd?:         number
  investmentAmountUsd?: number
  callNumber?:         number
  callPct?:            number   // e.g. 0.06 for 6%
  cumulativePct?:      number   // cumulative called pct e.g. 0.25
  dueDate?:            string
  fxRate?:             number
...
function extractLabelledUsd(text: string, ...labelPatterns: string[]): number | undefined {
  for (const label of labelPatterns) {
    const re = new RegExp(
      label.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') +
      '[^\\n\\$\\d]{0,30}(?:USD\\s*)?(\\$?[\\d,]+(?:\\.\\d{1,2})?)',
      'i'
    )
...
function extractLabelledPct(text: string, ...labelPatterns: string[]): number | undefined {
  for (const label of labelPatterns) {
    const re = new RegExp(
      label.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') +
      '[^\\n\\d]{0,20}([\\d.]+)\\s*%',
...
function extractDates(text: string): string[] {
  ...
  for (const m of text.matchAll(monthRe)) {
    ...
    if (mo) found.add(`${m[3]}-${mo}-${m[2].padStart(2,'0')}`)
  }
...
function scoreTypes(text: string): { cc: number; dist: number; nav: number } {
...
function detectNoticeType(text: string): 'capital_call' | 'distribution' | 'financial_statement' {
  ...
  if (cc === 0 && dist === 0 && nav === 0) return 'capital_call'
  if (cc >= dist && cc >= nav) return 'capital_call'
  if (dist >= cc && dist >= nav) return 'distribution'
  return 'financial_statement'
}

function isCombinedNotice(text: string): boolean {
  const t = text.toLowerCase()
  return (
    (t.includes('capital call') && t.includes('distribution')) &&
...
function extractLpId(text: string): string | undefined {
  const patterns = [
...
function extractWireInstructions(text: string): WireInstructions | undefined {
  const wire: WireInstructions = {}

  const bankM = text.match(/(?:Beneficiary\s+)?Bank(?:\s+Name)?\s*:?\s*([^\n]{4,60})/i)
  if (bankM) wire.bank = bankM[1].trim()

  const abaM = text.match(/ABA\s*:?\s*(\d{9})/i)
  if (abaM) wire.aba = abaM[1]

  const swiftM = text.match(/SWIFT\s*(?:\/BIC)?\s*:?\s*([A-Z0-9]{8,11})/i)
  if (swiftM) wire.swift = swiftM[1]

  const acctNameM = text.match(/Account\s+Name\s*:?\s*([^\n]{4,80})/i)
  if (acctNameM) wire.accountName = acctNameM[1].trim()

  const acctNumM = text.match(/Account\s+(?:Number|No\.?)\s*:?\s*([A-Z0-9\-]{4,30})/i)
  if (acctNumM) wire.accountNumber = acctNumM[1].trim()

  const refM = text.match(/(?:Wire\s+)?Reference\s*:?\s*([A-Za-z0-9\- ]{4,50}(?=\s*and\s+Your|\s*$|\n))/i)
  if (refM) wire.reference = refM[1].trim()

  return Object.keys(wire).length > 0 ? wire : undefined
}

function extractCommitmentSummary(text: string): CommitmentSummary | undefined {
  const summary: CommitmentSummary = {}

  summary.commitmentUsd = extractLabelledUsd(text,
    'Commitment Amount', 'Total Commitment', 'LP Commitment',
    'Capital Commitment', 'Your Commitment'
  )
  summary.totalCalledUsd = extractLabelledUsd(text,
    'Total Capital Called', 'Cumulative Capital Called',
    'Total Called', 'Capital Called to Date'
  )
  summary.unfundedUsd = extractLabelledUsd(text,
    'Unfunded Commitment', 'Remaining Commitment',
    'Remaining Unfunded', 'Undrawn Commitment'
  )
  summary.totalDistributionsUsd = extractLabelledUsd(text,
    'Total Distributions', 'Total Limited Partner Distributions',
    'Cumulative Distributions', 'Total Distributed'
  )

  // Percentages
  const calledPct = extractLabelledPct(text,
    'Total Capital Called', 'Cumulative Capital Called'
  )
  if (calledPct) summary.totalCalledPct = calledPct

  const unfundedPct = extractLabelledPct(text,
    'Unfunded Commitment', 'Remaining Commitment'
  )
  if (unfundedPct) summary.unfundedPct = unfundedPct
  ...
  if (dpiM) summary.distributionMultiple = parseFloat(dpiM[1])
...
const GEO_KEYWORDS: [RegExp, string][] = [
...
const DEAL_KEYWORDS: [RegExp, string][] = [
...
function inferSector(desc: string): string | undefined {
...
function inferGeography(desc: string): string | undefined {
  for (const [re, geo] of GEO_KEYWORDS) {
    if (re.test(desc)) return geo
  }
  return undefined
}

function inferDealType(desc: string): string | undefined {
  for (const [re, deal] of DEAL_KEYWORDS) {
    if (re.test(desc)) return deal
  }
  return undefined
}
...
function extractInvestmentTargets(text: string, dates: string[]): InvestmentTarget[] {
  ...
  for (const m of text.matchAll(distTableRe)) {
    ...
    if (!seen.has(projectName) && total > 0) {
      ...
      if (existing) {
        if (!existing.actualName && actualName.length > 3) existing.actualName = actualName
        if (!existing.amountUsd && total > 0) existing.amountUsd = total
      }
    }
  }
  ...
  for (const m of text.matchAll(
    /(?:Investment\s+(?:in|into)|Portfolio\s+Company|New\s+Investment|Follow-?On)[:\s–—]+([A-Z][A-Za-z0-9\s&,.'"\-–]{2,60})/gi
  )) {
  ...
  targets.forEach(t => {
    if (!('investmentDate' in t)) {
      (t as any).investmentDate = investDate ?? null
    }
  })
...
function extractCapitalCall(text: string, amounts: number[], dates: string[], combined: boolean) {
  ...
  result.grossCallUsd = extractLabelledUsd(text,
    ...
    'Gross Amount', 'Gross Contribution', '6.00% Capital Call'
  ) ?? amounts[0]

  // Net call — "Net Amount of Capital Call" / "Net Capital Call"
  result.netCallUsd = extractLabelledUsd(text,
    'Net Amount of Capital Call', 'Net Capital Call', 'Net Call Amount',
    'Net Drawdown', 'Amount Due', 'Amount Payable', 'Wire Amount',
    'Total Due', 'Total Payable', 'Please send your contribution of',
    'Net Amount'
  ) ?? (combined ? undefined : (amounts[1] ?? result.grossCallUsd))

  // Deemed distribution offset in combined notices
  if (combined) {
    result.deemedDistUsd = extractLabelledUsd(text,
      'Deemed Distribution', 'Less: Deemed Distribution', 'Distribution Offset',
      'Less Distribution', 'Gross Distribution', 'Net Distribution'
    )
    ...
    result.reinvestableUsd = extractLabelledUsd(text,
      'Reinvestable Amount', 'Recallable Amount', 'Amount Being Reinvested'
    )
  }

  // Components
  result.investmentAmountUsd = extractLabelledUsd(text,
    'Investment Amount', 'New Investment', 'Follow-On Investment', 'Equity Investment'
  )
  result.managementFeeUsd = extractLabelledUsd(text,
    'Management Fee', 'Advisory Fee', 'Fund Expenses', 'Management Fees'
  )
  result.expenseUsd = extractLabelledUsd(text,
    'Fund Expenses', 'Other Expenses', 'Organisation Costs', 'Administrative Fee'
  )
  ...
  if (!result.dueDate && dates.length > 0) result.dueDate = dates[0]
  ...
  for (const p of fxPatterns) {
    const m = text.match(p)
    if (m) { result.fxRate = parseFloat(m[1]); break }
  }
...
function extractDistribution(text: string, amounts: number[], dates: string[]) {
  const result: Partial<ExtractedNotice> = {}

  result.distributionUsd = extractLabelledUsd(text,
    'Total Distribution', 'Total:', 'Distribution Amount', 'Total Proceeds',
    'Gross Distribution', 'Net Distribution', '分配金総額',
    'Distribution Proceeds', 'Total Distributable Amount',
    'Total Secondary Investments'
  ) ?? amounts[0]

  const capitalReturn = extractLabelledUsd(text,
    'Return of Capital', 'Capital Return', 'Capital Returned',
    'Return of Invested Capital', 'Total.*Return of Capital'
  )
  const income = extractLabelledUsd(text,
    'Income Distribution', 'Realized Gain', 'Gain', 'Net Gain',
    'Capital Gain', 'Net Profit', 'Total.*Gain'
  )
  const recallable = extractLabelledUsd(text,
    'Recallable Amount', 'Reinvestable Amount', 'Amount Subject to Recall'
  )

  let totalUsd = result.distributionUsd
  let capRet   = capitalReturn
  let inc      = income
  if (capRet && inc && !totalUsd) totalUsd = capRet + inc
  if (totalUsd && capRet && !inc) inc      = totalUsd - capRet
  if (totalUsd && inc && !capRet) capRet   = totalUsd - inc

  // Extract distribution sources table
  const sources: DistributionBreakdown['sources'] = []
  ...
  if (dm) {
    ...
    if (parsed[0]) result.distributionDate = parsed[0]
  }
  if (!result.distributionDate && dates.length > 0) result.distributionDate = dates[0]
...
function extractFinancialStatement(text: string, amounts: number[], dates: string[]) {
  const result: Partial<ExtractedNotice> = {}
  ...
  for (const p of periodPatterns) {
    const m = text.match(p)
    if (m) { result.period = m[1].trim(); break }
  }
...
function computeConfidence(
  noticeType: string,
  extracted: Partial<ExtractedNotice>,
  textLen:   number
): { confidence: number; grade: 'high' | 'medium' | 'low' } {
  let score = 0
  if (textLen > 200)  score += 0.10
  if (textLen > 1000) score += 0.10

  if (noticeType === 'capital_call') {
    if (extracted.grossCallUsd)          score += 0.20
    if (extracted.netCallUsd)            score += 0.15
    if (extracted.dueDate)               score += 0.15
    if (extracted.wireInstructions?.aba) score += 0.10
    if (extracted.wireInstructions?.swift) score += 0.05
    if (extracted.commitmentSummary?.commitmentUsd) score += 0.10
    if (extracted.callPct)               score += 0.05
    if ((extracted.investmentTargets?.length ?? 0) > 0) score += 0.05
  } else if (noticeType === 'distribution') {
    if (extracted.distributionUsd)                       score += 0.25
    if (extracted.distributionBreakdown?.capitalReturnUsd) score += 0.15
    if (extracted.distributionBreakdown?.incomeUsd)      score += 0.10
    if (extracted.distributionDate)                      score += 0.10
    if ((extracted.distributionBreakdown?.sources?.length ?? 0) > 0) score += 0.10
  } else {
    if (extracted.navUsd)  score += 0.35
    if (extracted.navDate) score += 0.15
    if (extracted.period)  score += 0.10
    if (extracted.tvpi)    score += 0.10
    if (extracted.irr)     score += 0.05
  }

  const confidence = Math.min(score, 1.0)
  const grade: 'high' | 'medium' | 'low' = confidence >= 0.65 ? 'high' : confidence >= 0.35 ? 'medium' : 'low'
...
async function applyCustomExtractors(
  text:    string,
  base:    Record<string, any>,
): Promise<Record<string, any>> {
  try {
    const extractors = await prisma.attributeExtractor.findMany({ where: { isActive: true } })
    const custom: Record<string, any> = {}
    for (const ext of extractors) {
      const value = extractByKeyword(
        text,
        ext.keywords,
        ext.extractionType as 'usd' | 'pct' | 'number' | 'date' | 'text',
      )
      if (value !== undefined) {
...
export async function parsePdf(buffer: Buffer): Promise<ExtractedNotice> {
  ...
  let specific: Partial<ExtractedNotice> = {}
  if (noticeType === 'capital_call')
  ...
  return applyCustomExtractors(text, base) as Promise<ExtractedNotice>
}
