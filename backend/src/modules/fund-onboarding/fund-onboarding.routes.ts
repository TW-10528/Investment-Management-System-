...
import * as templateMgr from './template-manager';
import { calculateFileHash } from './pdf-labeler';
import {
  uploadRequestSchema,
...
router.post('/upload', async (c) => {
  try {
    ...
    if (!fileField || typeof fileField === 'string') {
      return c.json(
        { detail: 'No PDF file uploaded. Send multipart form with field "file".' },
        400
      );
    }
    ...
    // Initialize session
    const session = await controller.initializeSession(
      buffer,
      file.name,
      userEmail
    );
    ...
    return c.json(
      ...
      err.message?.includes('duplicate') ? 409 : 400
    );
  }
});
...
router.post('/sessions/:sessionId/classify', async (c) => {
  try {
    ...
    if (!fileField || typeof fileField === 'string') {
      return c.json(
        { detail: 'No PDF file. Send multipart form with field "file".' },
        400
      );
    }
    ...
    // Upload and classify
    const { session, classification } = await controller.uploadAndClassify(
      sessionId,
      buffer
    );
...
router.post('/sessions/:sessionId/extract', async (c) => {
  try {
    ...
    const validated = extractRequestSchema.partial().safeParse(reqData);
...
router.get('/sessions/:sessionId/calculated', async (c) => {
  try {
    ...
    if (!session.extractedValues) {
      return c.json(
        { detail: 'No extracted values yet. Complete extraction step first.' },
        400
      );
    }

    const extracted = session.extractedValues as Record<string, any>;
    const calculated = await controller.calculateValues(
      sessionId,
      extracted,
      session.fundKey || 'UNKNOWN'
    );
...
router.post('/sessions/:sessionId/validate', async (c) => {
  try {
    ...
    if (!session.extractedValues || !session.calculatedValues) {
      return c.json(
        { detail: 'Extraction and calculation must be complete first.' },
        400
      );
    }

    const extracted = session.extractedValues as Record<string, any>;
    const calculated = session.calculatedValues as any;
...
router.post('/sessions/:sessionId/save-template', async (c) => {
  try {
    ...
    if (!session.extractedValues) {
      return c.json(
        { detail: 'Extracted values required before saving template.' },
        400
      );
    }

    const reqData = await c.req.json();
    const validated = saveTemplateRequestSchema.safeParse(reqData);
    ...
    const userEmail = c.req.header('X-User-Email');
    const { templateName, fundKey, manager, strategy, isNewTemplate, templateId } =
      validated.data;

    const result = await controller.saveAsTemplate(
      sessionId,
      session.extractedValues as Record<string, any>,
      templateName,
      fundKey,
      {
        manager,
...
router.post('/sessions/:sessionId/complete', async (c) => {
  try {
    ...
    if (!session.templateId) {
      return c.json(
        { detail: 'Template must be saved before completing.' },
        400
      );
    }

    const userEmail = c.req.header('X-User-Email');
    const completed = await controller.completeOnboarding(
      sessionId,
      session.templateId,
      session.fileName,
      session.fileHash,
...
router.get('/templates', async (c) => {
  try {
    ...
    const { templates, total } = await templateMgr.listTemplates(
      limit,
      offset,
      search
    );
...
router.get('/templates/:templateId', async (c) => {
  try {
    ...
    return c.json({
      ...
      pdfLabels: (template.pdfLabels || []).map((label: any) => ({
...
router.post('/sessions/:sessionId/feedback', async (c) => {
  try {
    const sessionId = c.req.param('sessionId');
    const reqData = await c.req.json();
    const validated = correctionFeedbackRequestSchema.safeParse(reqData);
    ...
    const userEmail = c.req.header('X-User-Email');
    const { correctedFields, originalValues, correctedValues, feedback } =
      validated.data;

    await controller.recordCorrectionFeedback(
      sessionId,
      correctedFields,
      originalValues,
      correctedValues,
    ...
    return c.json({
      message:
        'Feedback recorded. AI will analyze this correction for template improvement.',
    });
  } catch (err: any) {
