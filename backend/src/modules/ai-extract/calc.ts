...
export interface PrevState {
  E: number   // cumulative contributions
  F: number   // unfunded commitment
  G: number   // cumulative net cash flow
}
...
export interface CalcResult {
  ...
  C: number
  D: number
  L: number           // C - D (non-recallable distribution)
  cash_flow: number   // -B + C
  E: number
  F: number
  G: number
...
export function applyCalcEngine(
  ext:      Extracted,
  fund_key: string,
  prev:     PrevState,
): CalcResult {
  ...
  let F = prev.F
  let G = prev.G
  let commitment_inferred: number | null = null
...
function parseNoteValue(notes: string, key: string): number | null {
