...
router.get('/summary', async (c) => {
  ...
  const overdueCalls = pendingCalls
    .filter((cc: any) => { const d = new Date(cc.dueDate); return d >= today && d <= todayEnd; })
  ...
  const irrFlows: { date: Date; amount: number }[] = [
    ...
    ...dists.map((d: any) => ({ date: d.distributionDate, amount: parseFloat(d.amountUsd.toString()) })),
  ...
  return c.json({
    ...
    overdue_calls: overdueCalls.map((cc: any) => ({
      id:           cc.id,
    ...
    recent_investments: recentInv.map((it: any) => ({
      id:              it.id,
      fund_id:         it.fundId,
