...
import { fundFamilyService } from '../../services/fundFamilyService';
...
app.get('/', async (c) => {
  try {
    const families = await fundFamilyService.listFundFamilies();
...
app.post('/', async (c) => {
  try {
    ...
    const family = await fundFamilyService.createFundFamily({
      familyName,
      familyCode,
      strategy,
      manager,
...
app.get('/:id', async (c) => {
  try {
    ...
    const family = await fundFamilyService.getFundFamily(id);
...
app.post('/add-fund', async (c) => {
  try {
    ...
    const family = await fundFamilyService.getOrCreateFundFamily({
      familyName,
      familyCode,
      strategy,
      manager,
...
app.get('/list/new-funds', async (c) => {
  try {
    const newFunds = await fundFamilyService.getNewFunds();
...
app.get('/list/existing-funds', async (c) => {
  try {
    const existingFunds = await fundFamilyService.getExistingFunds();
