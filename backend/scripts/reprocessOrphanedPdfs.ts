/**
 * Reprocess Orphaned PDFs
 *
 * Finds all PDF files in the uploads folder that don't have corresponding
 * Notice records, extracts their data, and creates the capital calls/distributions.
 *
 * Run: npx tsx scripts/reprocessOrphanedPdfs.ts
 */
...
async function reprocessOrphanedPdfs() {
  console.log('🔍 Scanning for orphaned PDFs...\n')
  ...
  for (const category of categories) {
    ...
    for (const file of files) {
      try {
        ...
        if (existing) {
          console.log(`⏭️  SKIPPED: ${file} (already processed as ${existing.id})`)
          skipped++
          continue
        }
        ...
        const relPath = path.join(category, file)
        ...
        await prisma.$transaction(async (tx) => {
          ...
          await tx.notice.create({
            data: {
              ...
              uploadedBy: 'batch-reprocess',
            },
          })
        })

        console.log(`   ✅ Created capital call/distribution for ${fund.fundName}`)
        processed++
