...
exports.validateExtractedNotice = validateExtractedNotice;
...
// ── Main export ───────────────────────────────────────────────────────────────
async function validateExtractedNotice(fundId, extractedData, noticeType) {
    const empty = (msg) => ({
    ...
    if (!fundId)
        return empty('No fund linked — historical validation skipped.');
    const fund = await prisma_1.prisma.fund.findUnique({
        where: { id: fundId },
        select: { id: true, fundName: true, commitmentUsd: true },
    });
    if (!fund)
        return empty('Fund not found in database.');
    // ── Pull historical approved/paid capital calls ───────────────────────────
    const pastCalls = await prisma_1.prisma.capitalCall.findMany({
        where: { fundId, status: { in: ['approved', 'paid'] } },
        orderBy: { dueDate: 'asc' },
    });
    const pastDists = await prisma_1.prisma.distribution.findMany({
        where: { fundId },
    ...
    if (noticeType === 'distribution') {
        const newDistAmt = parseFloat(String(extractedData.distributionUsd ?? extractedData.amounts?.[0] ??
    ...
    const overallRisk = hasError ? 'high' : hasWarning ? 'medium' : 'low';
    // ── Generate summary ──────────────────────────────────────────────────────
    const summary = buildSummary(fund.fundName, noticeType, callCount, flags, overallRisk, historicalContext, newGross, newFee);
    return {
        ran: true,
        overallRisk,
        flags,
...
function buildSummary(fundName, noticeType, callCount, flags, risk, ctx, newGross, newFee) {
    if (callCount === 0) {
