...
exports.extractCapulaGrvReport = extractCapulaGrvReport;
// ── Helpers ───────────────────────────────────────────────────────────────────
const MONTH_NUM = {
...
function findTextByLabel(text, labels) {
    for (const label of labels) {
...
function findDateByLabel(text, labels) {
    for (const label of labels) {
        const re = new RegExp(escapeRegex(label) + '\\s*:?\\s*([A-Za-z]{3}-\\d{1,2}-\\d{4}|\\d{1,2}/\\d{1,2}/\\d{4}|[A-Za-z]+\\s+\\d{1,2},\\s+\\d{4})', 'i');
        const m = text.match(re);
        if (m)
...
function mapToExcelFields(a, breakdown) {
    ...
    if (a.is_subscription)
        remarksParts.push('Subscription notice. Net Capital Contribution is used as capital contribution amount.');
    else if (a.is_distribution)
        remarksParts.push('Distribution notice. Distribution is used as investment received amount.');
    if (a.capital_balance != null)
        remarksParts.push('Capital Balance is extracted separately as NAV/capital balance and is not used as cumulative contribution.');
    return {
...
function calculateExcelFields(excel, a, previousState = null) {
    ...
    let cumulativeContributions = a.report_cumulative_capital_contributions;
    let remainingCommitment = a.report_remaining_commitment;
...
// ── Validation ─────────────────────────────────────────────────────────────────
function buildValidation(excel, a, _breakdown, calculationResult) {
...
// ── Main ───────────────────────────────────────────────────────────────────────
function extractCapulaGrvReport(rawText, fileName = '', previousState = null) {
    ...
    if (allFields.is_subscription)
        documentType = 'subscription_notice';
    else if (allFields.is_distribution)
