...
exports.extractHamiltonReport = extractHamiltonReport;
...
function detectDocumentType(text) {
    ...
    if (lower.includes('re: distribution') || lower.includes('distribution amount:'))
        return 'distribution_notice';
    if (lower.includes('re: capital call') || lower.includes('capital call amount:'))
...
function extractAllFields(text) {
    ...
    if (capitalCallForInvestments == null) {
        ...
        if (investMatch)
            capitalCallForInvestments = cleanAmount(investMatch[1], true) ?? null;
    }
    ...
    if (capitalCallForManagementFees == null) {
        ...
        if (feeMatch)
            capitalCallForManagementFees = cleanAmount(feeMatch[1], true) ?? null;
    }
    ...
    if (capitalCallForExpenses == null) {
        ...
        if (expMatch)
            capitalCallForExpenses = cleanAmount(expMatch[1], true) ?? null;
    }
    ...
    let capitalContributionAmount = round2(capitalComponentValues.reduce((s, v) => s + (v ?? 0), 0));
    ...
    const reinvestableAmount = round2(amountOrZero(distReturnCapitalRecallable)
...
function mapToExcelFields(a, breakdown) {
    ...
    if (a.document_type === 'capital_call_notice') {
        ...
        if (a.capital_call_for_investments != null)
            remarksParts.push(`Capital ${num(a.capital_call_for_investments)}.`);
        if (a.capital_call_for_management_fees != null)
            remarksParts.push(`Management fee ${num(a.capital_call_for_management_fees)}.`);
        if (a.capital_call_for_expenses != null)
            remarksParts.push(`Expense ${num(a.capital_call_for_expenses)}.`);
        if (a.subsequent_close_interest_payable != null)
            remarksParts.push(`Subsequent close interest payable ${num(a.subsequent_close_interest_payable)}; excluded from Excel cash flow.`);
        if (a.subsequent_close_interest_receivable != null)
            remarksParts.push(`Subsequent close interest receivable ${num(a.subsequent_close_interest_receivable)}; excluded from Excel cash flow.`);
    }
...
function buildValidation(excelFields, a, breakdown, calculationResult) {
    ...
    const capitalCallBreakdownTotalExcel = round2(breakdown.capital_call_breakdown
...
function extractHamiltonReport(rawText, fileName = '', previousState = null) {
