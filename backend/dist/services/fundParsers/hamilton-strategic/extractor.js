...
exports.extractHamiltonStrategicReport = extractHamiltonStrategicReport;
...
function findAccountingTreatmentAmount(text, label) {
    const sectionMatch = text.match(/Current\s+Distribution\s+Accounting\s+Treatment[\s\S]{0,1200}?Inception-to-Date\s+Activity/i);
...
function extractAllFields(text) {
    ...
    if (returnUnusedCapitalForInvestments == null) {
        ...
        if (m)
            returnUnusedCapitalForInvestments = cleanAmount(m[1], true);
    }
    ...
    if (subsequentCloseInterestReceivable == null) {
        ...
        if (m)
            subsequentCloseInterestReceivable = cleanAmount(m[1], true);
    }
    ...
    if (returnUnusedCapitalForInvestments != null) {
        ...
        const componentSum = componentValues.reduce((s, v) => s + (v ?? 0), 0);
        if (componentSum)
            capitalContributionAmountForExcel = round2(componentSum);
        else if (capitalCallAmountHeader != null)
            capitalContributionAmountForExcel = capitalCallAmountHeader;
        else
    ...
    if (returnUnusedCapitalForInvestments != null) {
        ...
        if (distributionTotalForExcel)
            distributionAmountReceivedForExcel += distributionTotalForExcel;
        if (subsequentCloseInterestReceivable)
            distributionAmountReceivedForExcel += subsequentCloseInterestReceivable;
    ...
    if (actualPaymentAmount == null) {
        if (capitalCallAmountHeader != null)
            actualPaymentAmount = capitalCallAmountHeader;
        else if (distributionAmountHeader != null)
            actualPaymentAmount = -distributionAmountHeader;
    }
    return {
...
function buildBreakdown(a) {
    ...
    for (const [purpose, label, amount] of componentMap) {
        if (amount != null)
            capital_call_breakdown.push({ purpose, label, amount, excel_usage: 'capital_contribution_amount_component' });
    }
...
function mapToExcelFields(a, breakdown) {
    ...
    if (b > 0 && c > 0)
        remarksParts.push('Net capital call: capital call is netted against distribution.');
    else if (b > 0)
        remarksParts.push('Capital call transaction.');
    else if (b < 0)
        remarksParts.push('Return of unused capital reduces cumulative capital contributions.');
    else if (c > 0)
        remarksParts.push('Distribution transaction.');
    if (a.subsequent_close_interest_receivable)
        remarksParts.push('Subsequent close interest receivable is included in distribution amount received but not reinvestable amount.');
    return {
        ...
        gain: 0,
        interest: round2(amountOrZero(a.interest_income)
            + amountOrZero(a.other_investment_income)
            + amountOrZero(a.subsequent_close_interest_receivable)),
        interest_other: round2(amountOrZero(a.interest_income)
            + amountOrZero(a.other_investment_income)
            + amountOrZero(a.subsequent_close_interest_receivable)),
...
function buildValidation(excelFields, a, breakdown, calculationResult) {
    ...
    const capitalCallBreakdownTotal = round2(breakdown.capital_call_breakdown
        .filter(i => i.amount != null && i.purpose !== 'subsequent_close_interest_payable')
        .reduce((s, i) => s + i.amount, 0));
    const distributionBreakdownTotal = round2(breakdown.distribution_breakdown.filter(i => i.amount != null).reduce((s, i) => s + i.amount, 0));
    const reportE = a.amounts_drawn;
...
function extractHamiltonStrategicReport(rawText, fileName = '', previousState = null) {
    ...
    if (fb > 0 && fc > 0)
        documentType = 'net_capital_call_notice';
    else if (fb > 0)
        documentType = 'capital_call_notice';
    else if (fb < 0)
        documentType = 'return_of_unused_capital_notice';
    else if (fc > 0)
