...
exports.extractGoldmanVintageXReport = extractGoldmanVintageXReport;
// ── Helpers ───────────────────────────────────────────────────────────────────
const MONTH_NUM = {
...
function findDueDate(text) {
    ...
    let m = text.match(new RegExp(`due\\s+${days},?\\s*([A-Za-z]+\\s+\\d{1,2},\\s+\\d{4})`, 'i'));
    if (m)
        return normalizeDate(m[1]);
    // "by Monday, February 02, 2026"
    m = text.match(new RegExp(`by\\s+${days},?\\s*([A-Za-z]+\\s+\\d{1,2},\\s+\\d{4})`, 'i'));
    if (m)
        return normalizeDate(m[1]);
    return null;
}
...
function extractAllFields(text) {
    ...
    const fundingM1 = text.match(/Your\s+portion\s+of\s+the\s+capital\s+contribution\s+is\s+as\s+follows:[\s\S]*?Amount\s*\$?\s*([\d,]+(?:\.\d+)?)/i);
    let fundingAmount = fundingM1 ? cleanAmount(fundingM1[1], true) : null;
...
function mapToExcelFields(a, breakdown) {
    ...
    const capitalContributionAmount = a.gross_contribution ?? a.secondary_investments ?? a.funding_amount ?? 0;
    ...
    const remarks = 'Goldman Sachs Vintage X capital contribution notice. ' +
...
function calculateExcelFields(excel, a, previousState = null) {
    ...
    let cumulativeContributions = reportE;
...
function buildValidation(excel, a, _breakdown, calculationResult) {
    ...
    const reportE = a.contributions_to_date;
    const reportF = a.outstanding_commitment;
    let commitmentMinusE = null;
    let commitmentMinusEMatchesF = null;
    if (commitment != null && reportE != null) {
        commitmentMinusE = round2(commitment - reportE);
        if (reportF != null)
            commitmentMinusEMatchesF = round2(commitmentMinusE) === round2(reportF);
    }
...
// ── Main ───────────────────────────────────────────────────────────────────────
function extractGoldmanVintageXReport(rawText, fileName = '', previousState = null) {
