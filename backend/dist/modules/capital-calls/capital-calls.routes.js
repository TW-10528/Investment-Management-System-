...
router.get('/', async (c) => {
    ...
    if (status)
        where.status = status;
    const calls = await prisma_1.prisma.capitalCall.findMany({
        where,
...
router.get('/:id', async (c) => {
    const cc = await prisma_1.prisma.capitalCall.findUnique({
...
router.post('/', async (c) => {
    ...
    const { fund_id, notice_date, due_date, execution_date, call_number, call_pct = 0, gross_call_usd = 0, distribution_usd = 0, reinvestable_usd = 0, net_call_usd, fx_rate, investment_amount_usd = 0, management_fee_usd = 0, expense_usd = 0, wire_reference, wire_fee_jpy = 0, is_recallable = false, notes, initial_status, } = body;
...
router.patch('/:id', async (c) => {
    const cc = await prisma_1.prisma.capitalCall.findUnique({ where: { id: c.req.param('id') } });
    if (!cc)
        return c.json({ detail: 'Not found' }, 404);
    ...
    if (b.execution_date !== undefined)
        data.executionDate = b.execution_date ? new Date(b.execution_date) : null;
    if (b.notes !== undefined)
        data.notes = b.notes;
    if (b.wire_reference !== undefined)
        data.wireReference = b.wire_reference;
    if (b.wire_fee_jpy !== undefined)
        data.wireFeeJpy = parseFloat(b.wire_fee_jpy);
    if (b.is_recallable !== undefined)
        data.isRecallable = Boolean(b.is_recallable);
    if (b.fx_rate !== undefined)
        data.fxRate = parseFloat(b.fx_rate);
    ...
    const updated = await prisma_1.prisma.capitalCall.update({ where: { id: cc.id }, data });
...
router.delete('/:id', async (c) => {
    ...
    if (!cc)
        return c.json({ detail: 'Not found' }, 404);
    await prisma_1.prisma.capitalCall.delete({ where: { id: cc.id } });
    return c.json({ ok: true });
});
exports.default = router;
//# sourceMappingURL=capital-calls.routes.js.map