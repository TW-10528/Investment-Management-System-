...
// ── Serialiser ────────────────────────────────────────────────────────────────
function reportDict(n) {
    const d = n.extractedData ?? {};
    return {
        id: n.id,
...
router.post('/upload', async (c) => {
    ...
    let file = null;
    let extractionDataStr = null;
    let customDocType = null;
    try {
        const body = await c.req.parseBody();
        file = body['file'];
        extractionDataStr = body['extraction_data'] ?? null;
        customDocType = body['custom_doc_type'] ?? null;
    }
    catch {
    ...
    const existing = await prisma_1.prisma.notice.findFirst({ where: hashScope });
    if (existing) {
    ...
    if (reqTypePre === 'commitment_notice' && scopedFundIdPre) {
        const commitFund = await prisma_1.prisma.fund.findUnique({ where: { id: scopedFundIdPre } });
        if (!commitFund)
        ...
        const folderPath = path_1.default.join(index_6.config.uploadDir, fundFolder);
        if (!fs_1.default.existsSync(folderPath))
            fs_1.default.mkdirSync(folderPath, { recursive: true });
        const relPath = path_1.default.join(fundFolder, `${Date.now()}_${safe}`);
        const filepath = path_1.default.join(index_6.config.uploadDir, relPath);
        ...
        if (commitmentUsdStr) {
            ...
            if (amt > 0) {
                await prisma_1.prisma.fund.update({
                    ...
                    data: { commitmentUsd: new decimal_js_1.default(amt) },
                });
            }
        }
        const n = await prisma_1.prisma.notice.create({
            data: {
                filename: relPath,
                originalName,
                fileHash,
    ...
    let viewingDocCheck = null;
    if (!hasExplicitType) {
        let pdfText = '';
        try {
            const { extractPdfText } = await Promise.resolve().then(() => __importStar(require('../../modules/ai-extract/ocr')));
        ...
        viewingDocCheck = (0, viewingDocumentDetector_1.detectViewingDocument)(pdfText, originalName);
    ...
    if (isViewingDoc || (hasExplicitType && ['financial_statement', 'nav_report', 'quarterly_report', 'annual_report', 'tax_document', 'audit_report', 'other_document', 'viewing_document', 'commitment_notice'].includes(reqType))) {
        ...
        if (scopedFundIdPre) {
            resolvedFund = await prisma_1.prisma.fund.findUnique({ where: { id: scopedFundIdPre } });
        }
        if (!resolvedFund) {
            // Fallback: try to resolve by auto-detection or use SDG
            resolvedFund = await (0, fund_resolver_1.resolveFund)('sdg-lps');
        }
        if (!resolvedFund) {
            return c.json({
    ...
    let parsed = await (0, index_1.parseFundPdf)(buffer, originalName);
    ...
    if (extractionDataStr) {
        try {
            const aiExtracted = JSON.parse(extractionDataStr);
    ...
    const resolvedFund = await (0, fund_resolver_1.resolveFund)(parsed.fundKey);
    if (!resolvedFund) {
        return c.json({
    ...
    let commitmentId = c.req.query('commitment_id') || null;
    if (commitmentId) {
        const cm = await prisma_1.prisma.commitment.findUnique({ where: { id: commitmentId } });
    ...
    if (RICH_FUNDS.includes(parsed.fundKey) && parsed.rawText) {
        ...
        const reparsed = parsed.fundKey === 'nb-real-estate' ? (0, index_2.parseNbRealEstate)(parsed.rawText, previousState)
            : parsed.fundKey === 'hamilton-lane' ? (0, index_3.parseHamiltonLane)(parsed.rawText, previousState)
                : parsed.fundKey === 'hamilton-strategic' ? (0, index_4.parseHamiltonStrategic)(parsed.rawText, previousState)
                    : (0, index_5.parseDoverStreet)(parsed.rawText, previousState, originalName);
    ...
    if (!fs_1.default.existsSync(folderPath))
        fs_1.default.mkdirSync(folderPath, { recursive: true });
    const relPath = path_1.default.join(fundFolder, `${Date.now()}_${safe}`); // stored on the notice; delete reuses it
    const filepath = path_1.default.join(index_6.config.uploadDir, relPath);
    fs_1.default.writeFileSync(filepath, buffer);
    ...
    if (customDocType) {
        storedData.customDocTypeName = customDocType;
    }
    ...
    const noticeType = parsed.noticeType === 'capital_and_distribution' ? parsed.noticeType
        ...
        const result = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            const n = await tx.notice.create({
                data: {
                    ...
                    extractedData: { ...storedData, createdCallId: made.callId ?? null, createdDistId: made.distId ?? null },
    ...
    if (FUNDS_WITH_COMMITMENT_IN_REPORT.includes(parsed.fundKey) && (parsed.commitmentUsd ?? 0) > 0) {
        ...
        if (Math.abs(parsed.commitmentUsd - currentCommitment) > 1) {
            await prisma_1.prisma.fund.update({
                ...
                data: { commitmentUsd: new decimal_js_1.default(parsed.commitmentUsd) },
            });
        }
    }
    ...
    const extracted = notice.extractedData ?? {};
    return c.json({
        ...reportDict(notice),
...
async function reconcileOrphanedNotices() {
    ...
    if (!fs_1.default.existsSync(index_6.config.uploadDir))
        return;
    const notices = await prisma_1.prisma.notice.findMany();
    for (const n of notices) {
        if (!n.filename)
            continue; // no backing file to track
        const filepath = path_1.default.join(index_6.config.uploadDir, n.filename);
        if (fs_1.default.existsSync(filepath))
            continue; // file still present → keep
        // The stored PDF is gone from disk → reverse it like a normal delete.
        const d = n.extractedData ?? {};
        try {
            if (d.createdCallId)
                await prisma_1.prisma.capitalCall.deleteMany({ where: { id: d.createdCallId } });
            if (d.createdDistId)
                await prisma_1.prisma.distribution.deleteMany({ where: { id: d.createdDistId } });
            await prisma_1.prisma.notice.delete({ where: { id: n.id } });
...
router.get('/', async (c) => {
    ...
    try {
        await reconcileOrphanedNotices();
    }
    catch (e) {
        console.error('[fund-reports] reconcile error (non-fatal):', e);
    }
    ...
    if (fund_id)
        results = results.filter(n => n.fundId === fund_id);
    if (fund_key)
        results = results.filter(n => n.extractedData?.fundKey === fund_key);
...
async function latestFundPreviousState(fundId, commitmentId = null) {
    // When a commitment is selected, chain only within that commitment so each
    // commitment's running totals are independent.
    const notices = await prisma_1.prisma.notice.findMany({ where: { fundId, commitmentId } });
    const docs = notices
        .filter(n => n.extractedData?.fundReport?.final_excel_fields)
        .sort((a, b) => docDate(b) - docDate(a)); // newest first
    const f = docs[0]?.extractedData?.fundReport?.final_excel_fields;
    if (!f)
        return null;
    return {
...
router.get('/:id', async (c) => {
    ...
    const fundReport = n.extractedData?.fundReport ?? null;
...
router.get('/:id/file', async (c) => {
    ...
    const filepath = path_1.default.join(index_6.config.uploadDir, n.filename);
    if (!fs_1.default.existsSync(filepath))
        return c.json({ detail: 'File not found on disk' }, 404);
    const buf = fs_1.default.readFileSync(filepath);
    const contentType = FILE_CONTENT_TYPES[path_1.default.extname(n.filename).toLowerCase()] ?? 'application/pdf';
...
router.post('/:id/approve', async (c) => {
    ...
    if (notice.status === 'approved')
        return c.json({ detail: 'Already approved' }, 400);
    if (!notice.fundId)
        return c.json({ detail: 'No fund linked to this report.' }, 400);
    const fund = await prisma_1.prisma.fund.findUnique({ where: { id: notice.fundId } });
    if (!fund)
        return c.json({ detail: 'Linked fund not found.' }, 404);
    const d = notice.extractedData ?? {};
    ...
    if (notice.noticeType === 'capital_call') {
        ...
        const existing = await prisma_1.prisma.capitalCall.findFirst({
        ...
        if (existing) {
            ...
            const last = await prisma_1.prisma.capitalCall.findFirst({ where: { fundId: notice.fundId }, orderBy: { callNumber: 'desc' } });
            ...
            for (const it of (d.investmentTargets ?? [])) {
                await prisma_1.prisma.investmentTarget.create({
        ...
        const existing = await prisma_1.prisma.distribution.findFirst({
    ...
    const txns = [
        ...paidCalls.map((cc) => ({
            ...
            capitalPaidIn: new decimal_js_1.default(cc.grossCallUsd.toString()), // B
            capitalReceived: new decimal_js_1.default(cc.distributionUsd.toString()), // C
            reinvestable: new decimal_js_1.default(cc.reinvestableUsd.toString()), // D
        ...
        ...distributions.map((dist) => ({
            ...
            fxRate: dist.fxRate ? new decimal_js_1.default(dist.fxRate.toString()) : null,
            capitalPaidIn: new decimal_js_1.default(0),
            capitalReceived: new decimal_js_1.default(dist.amountUsd.toString()), // C
            reinvestable: new decimal_js_1.default(dist.reinvestableUsd.toString()), // D
        })),
    ];
    let snapshot = null;
    let ledgerRows = [];
...
router.delete('/:id', async (c) => {
    ...
    if (notice.filename) {
        const filepath = path_1.default.join(index_6.config.uploadDir, notice.filename);
        try {
            if (fs_1.default.existsSync(filepath))
                fs_1.default.unlinkSync(filepath);
        }
    ...
    await prisma_1.prisma.notice.delete({ where: { id: notice.id } });
...
router.get('/:id/ledger', async (c) => {
    const notice = await prisma_1.prisma.notice.findUnique({ where: { id: c.req.param('id') } });
    if (!notice || !notice.fundId)
        return c.json({ detail: 'No fund linked to this report.' }, 404);
    const fund = await prisma_1.prisma.fund.findUnique({ where: { id: notice.fundId } });
    if (!fund)
        return c.json({ detail: 'Fund not found.' }, 404);
    const [paidCalls, distributions, commitmentHistory] = await Promise.all([
    ...
    const txns = [
        ...
        ...distributions.map((dist) => ({
            date: dist.distributionDate, txType: 'distribution', description: dist.distType,
            fxRate: dist.fxRate ? new decimal_js_1.default(dist.fxRate.toString()) : null,
            capitalPaidIn: new decimal_js_1.default(0), capitalReceived: new decimal_js_1.default(dist.amountUsd.toString()),
            reinvestable: new decimal_js_1.default(dist.reinvestableUsd.toString()),
        })),
    ];
    if (txns.length === 0)
...
//# sourceMappingURL=fund-reports.routes.js.map